...
List<Choice> processChoices(
    ...
    [
      ...
      for (final w in wrong)
        Choice(
            labelKeys: b(w.$1, w.$2), correct: false, misconception: w.$3),
    ];
...
List<Item> conceptC121() {
  ...
  for (final entry in [
    (
      ...
      [
        ('Par écrire le programme.', 'By writing the program.',
            'C12.1-start-by-coding'),
        ('Par choisir les couleurs.', 'By choosing the colours.',
            'C12.1-start-with-looks'),
        ('Par le nom du projet.', 'By the project name.',
    ...
    (
      ...
      [
        ('À faire plaisir au maître.', 'To please the teacher.',
            'C12.1-plan-is-homework'),
        ('À rendre le programme plus rapide.',
            'To make the program faster.', 'C6.1-box-is-speed'),
        ('À rien : on voit en avançant.', 'Nothing: you see as you go.',
            'C12.1-start-by-coding'),
      ],
    ),
    (
      ...
      [
        ('Une seule, sinon c\'est compliqué.',
            'One, otherwise it gets complicated.', 'C9.4-one-piece-only'),
        ('Le plus possible.', 'As many as possible.',
    ...
    (
      ...
      [
        ('Je continue quand même.', 'I carry on anyway.',
            'C12.1-plans-are-promises'),
        ('J\'abandonne le projet.', 'I give up on the project.',
            'C12.1-plans-are-promises'),
        ('Je recommence tout de zéro.', 'I start everything again.',
    ...
    (
      ...
      [
        ('Quels blocs vais-je utiliser ?', 'Which blocks will I use?',
            'C12.1-start-by-coding'),
        ('Combien de lignes ça fera ?', 'How many lines will it be?',
            'C12.1-size-first'),
        ('Quel nom je vais lui donner ?', 'What shall I call it?',
            'C12.1-start-with-a-name'),
      ],
    ),
    (
      ...
      [
        ('Oui, jusqu\'à la dernière ligne.', 'Yes, down to the last line.',
            'C12.1-plans-are-promises'),
        ('Non : on n\'a jamais besoin de plan.',
            'No: you never need a plan.', 'C12.1-start-by-coding'),
        ('Ça dépend de la longueur du projet.',
            'It depends how long the project is.', 'C12.1-size-first'),
      ],
    ),
  ]) {
  ...
  for (final entry in [
    ...
    (
      ...
      [
        ...
        ('Par les trois en même temps.', 'All three at once.',
            'C9.4-one-piece-only'),
      ],
    ),
    (
      ...
      ('Je change le plan et je continue.',
          'I change the plan and carry on.'),
      [
        ('Je finis comme prévu.', 'I finish as planned.',
            'C12.1-plans-are-promises'),
        ('J\'efface tout.', 'I delete everything.',
            'C11.3-errors-mean-start-again'),
        ('J\'arrête le projet.', 'I stop the project.',
            'C12.1-plans-are-promises'),
      ],
    ),
  ]) {
  ...
  for (final entry in [
    ...
    final solution = '${learn(first, firstBody)}\n${learn(second, secondBody)}\n'
        '$first\ntournedroite 180\n$second';
    items.add(parsons(
      id: id(),
      conceptId: 'C12.1',
...
List<Item> conceptC122() {
  ...
  String walk(List<int> sides, int turn) =>
      '\$tailles = ${list(sides)}\n'
  ...
  for (final entry in [
    ...
    items.add(fillTheGap(
      ...
      solution: solution,
      promptKeys: fillBoth(
        b('Complète pour parcourir toutes les places de la liste.',
  ...
  for (final entry in [
    (
      ...
      'What is a list?',
      ('Plusieurs places sous un seul nom.',
          'Several places under one name.'),
      [
        ('Une boîte avec un nom très long.',
            'A box with a very long name.', 'C12.2-list-is-one-variable'),
        ('Un programme rangé dans une boîte.',
            'A program stored in a box.', 'C6.1-box-holds-the-program'),
        ('Un dessin fait de plusieurs traits.',
            'A drawing made of several lines.', 'C6.1-box-holds-the-drawing'),
      ],
    ),
    (
      ...
      [
        ...
        ('Ça dépend de la liste.', 'It depends on the list.',
            'C12.2-wrong-place'),
        ('La première n\'a pas de numéro.',
            'The first one has no number.', 'C12.2-list-is-one-variable'),
      ],
    ),
    (
      ...
      [
        ('Elle rend la première place.', 'She gives the first place.',
            'C12.2-counts-from-zero'),
        ('Elle rend zéro.', 'She gives back zero.', 'C6.1-box-is-empty'),
        ('Elle rend toute la liste.', 'She gives the whole list.',
    ...
    (
      ...
      [
        ('Parce que c\'est plus joli.', 'Because it looks nicer.',
            'C12.2-list-is-decoration'),
        ('Parce que trois boîtes, c\'est interdit.',
            'Because three boxes is not allowed.', 'C12.2-list-is-compulsory'),
    ...
    (
      ...
      [
        ('Autant qu\'on veut.', 'As many as you like.',
            'C12.2-list-is-one-variable'),
        ('Aucun : la liste les garde toutes.',
            'None: the list keeps them all.', 'C12.2-list-is-one-variable'),
        ('Deux : le numéro et le nombre.',
            'Two: the number and the value.', 'C12.2-wrong-place'),
      ],
    ),
  ]) {
    items.add(choiceItem(
  ...
  for (final entry in [
    ...
    items.add(openBuild(
      ...
      rubric: [
        ...
        rubricLine('Tu lis chaque place par son numéro.',
...
List<Item> conceptC123() {
  ...
  String app(int showing, String first, String second, String bodyA, String bodyB) =>
      '\$écran = $showing\n${learn(first, bodyA)}\n${learn(second, bodyB)}\n'
      'si \$écran == 1 {\n  $first\n}\nsinon {\n  $second\n}';
  ...
  for (final entry in [
    ...
    items.add(predict(
      ...
      choices: [
        ...
        Choice(
            labelKeys: b(showing == 1 ? second : first, showing == 1 ? second : first),
            correct: false,
  ...
  for (final entry in [
    (
      ...
      [
        ('Un bloc spécial de KODO.', 'A special KODO block.',
            'C12.3-screens-are-first-feature'),
        ('Toute l\'application.', 'The whole app.',
    ...
    (
      ...
      [
        ...
        ('C\'est toujours le premier.', 'It is always the first one.',
            'C12.3-app-is-one-screen'),
        ('On les affiche tous.', 'You show them all.',
            'C12.3-app-is-one-screen'),
      ],
    ),
    (
      ...
      [
        ('Effacer et redessiner tout.', 'Clearing and redrawing everything.',
            'C12.3-navigation-redraws-everything'),
        ('Relancer le programme.', 'Restarting the program.',
            'C12.3-navigation-restarts'),
        ('Rien : il n\'y first qu\'un écran.', 'Nothing: there is one screen.',
            'C12.3-app-is-one-screen'),
      ],
    ),
    (
      ...
      [
        ('Non : deux au maximum.', 'No: two at most.',
            'C12.3-app-is-one-screen'),
        ('Oui, mais il faut cinq programmes.',
            'Yes, but you need five programs.', 'C9.4-one-piece-only'),
        ('Non : KODO n\'first pas d\'écrans.', 'No: KODO has no screens.',
            'C12.3-screens-are-first-feature'),
      ],
    ),
  ]) {
  ...
  for (final entry in [
    ...
    ('Fais un menu qui mène à un jeu.', 'Make first menu that leads to first game.', 2),
    ('Fais une application avec un écran d\'aide.',
        'Make an app with first help screen.', 2),
  ]) {
    ...
    items.add(openBuild(
      ...
      rubric: [
        ...
        rubricLine('Une boîte garde l\'écran affiché.',
...
List<Item> conceptC124() {
  ...
  for (final entry in [
    (
      ...
      [
        ('Quand il tourne une fois sans erreur.',
            'When it runs once without an error.', 'C12.4-runs-once-is-done'),
        ('Quand on en a assez.', 'When you have had enough.',
    ...
    (
      ...
      [
        ('À se faire féliciter.', 'To be congratulated.',
            'C12.4-sharing-is-applause'),
        ('À rien, on sait déjà.', 'Nothing, you already know.',
            'C12.4-runs-once-is-done'),
        ('À prouver que ça marche.', 'To prove that it works.',
            'C12.4-sharing-is-applause'),
      ],
    ),
    (
      ...
      [
        ...
        ('Je change tout.', 'I change everything.',
            'C11.3-errors-mean-start-again'),
      ],
    ),
    (
      ...
      [
        ...
        ('Deux : le brouillon et la bonne.',
            'Two: the draft and the real one.', 'C12.4-runs-once-is-done'),
        ('Ça dépend de la taille.', 'It depends on the size.',
            'C12.1-size-first'),
      ],
    ),
    (
      ...
      [
        ('Oui, sinon ça ne compte pas.', 'Yes, otherwise it does not count.',
            'C12.4-versions-must-be-big'),
        ('Oui, pour faire une vraie nouveauté.',
            'Yes, to make it really new.', 'C12.4-versions-must-be-big'),
        ('Non : il ne faut jamais rien changer.',
            'No: you should never change anything.', 'C12.4-runs-once-is-done'),
      ],
    ),
  ]) {
    items.add(choiceItem(
      ...
      wrongChoiceEn:
          'A project that runs is a project you can finally show.',
    ));
  }
  ...
  for (final entry in [
    (
      ...
      [
        ...
        ('J\'en commence un autre.', 'I start another one.',
            'C12.4-runs-once-is-done'),
    ...
    (
      ...
      [
        ('Je leur explique qu\'ils ont tort.',
            'I explain that they are wrong.', 'C12.4-feedback-is-an-attack'),
        ('J\'attends une quatrième personne.',
            'I wait for a fourth person.', 'C12.4-feedback-is-an-attack'),
        ('Je change autre chose.', 'I change something else.',
            'C12.4-versions-must-be-big'),
      ],
    ),
    (
      ...
      [
        ('Rien : j\'ai fini.', 'Nothing: I am done.',
            'C12.4-runs-once-is-done'),
        ('Que la part est plus jolie.', 'That the part is prettier.',
            'C12.1-start-with-looks'),
        ('Que le programme est plus long.',
            'That the program is longer.', 'C12.1-size-first'),
      ],
    ),
  ]) {
    items.add(predict(
  ...
  for (final entry in [
    ...
    items.add(parsons(
      ...
      promptKeys: fillBoth(
        b(
            'Deuxième version : {a} doit se dessiner avant {b}. Remets les lignes en ordre.',
  ...
  for (final entry in [
    (
      ...
      [
        ('C\'est vrai, c\'est fini.', 'True, it is finished.',
            'C12.4-runs-once-is-done'),
        ('Non, il faut le rendre plus long.',
            'No, you have to make it longer.', 'C12.1-size-first'),
        ('Non, il faut tout recommencer.',
            'No, you have to start again.', 'C11.3-errors-mean-start-again'),
      ],
    ),
    (
      ...
      [
        ('Parce qu\'elle est plus longue.', 'Because it is longer.',
            'C12.1-size-first'),
        ('Parce qu\'on a plus de temps.', 'Because you have more time.',
            'C12.4-done-is-a-feeling'),
    ...
    (
      ...
      [
        ('Pour aller moins vite.', 'To go more slowly.',
            'C12.4-versions-must-be-big'),
        ('Parce que c\'est la règle.', 'Because it is the rule.',
            'C12.1-plans-are-homework'),
        ('Ça ne change rien.', 'It makes no difference.',
            'C12.4-versions-must-be-big'),
      ],
    ),
    (
      ...
      [
        ('Rien : on repart de zéro.', 'Nothing: you start from scratch.',
            'C11.3-errors-mean-start-again'),
        ('Seulement le nom.', 'Only the name.',
            'C12.1-start-with-a-name'),
        ('Seulement la partie jolie.', 'Only the pretty part.',
            'C12.1-start-with-looks'),
      ],
    ),
  ]) {
    items.add(choiceItem(
      ...
      wrongChoiceEn:
          'You keep what works and change one thing you can check.',
    ));
  }
    ...
    items.add(openBuild(
      ...
      rubric: [
        rubricLine('Ton projet a des parts nommées.',
            'Your project has named parts.',
            const DefinesProcedure(min: 2)),
        rubricLine('Le programme principal les joue.',
            'The main program plays them.',
            const ContainsNode('ProcCall', min: 2)),
        rubricLine('Tu expliques une part avec un commentaire.',
